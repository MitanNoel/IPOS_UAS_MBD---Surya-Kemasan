...
<head>
    ...
    <title>Edit Pembelian | IPOS Toko Surya Kemasan</title>
</head>
<body class="bg-slate-50 text-slate-800 min-h-screen">
    ...
    <div class="main-content px-4 py-8 fade-in">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
                        <i data-lucide="edit" class="w-6 h-6 text-brand-600"></i>
                        Edit Nota Pembelian
                    </h1>
                    <p class="text-xs text-slate-500">Ubah data transaksi pengadaan barang untuk nota pembelian #<?= htmlspecialchars($purchase['id_pembelian']) ?>.</p>
                </div>
            ...
            <div class="mb-4 p-4 bg-red-50 border border-red-100 rounded-xl text-red-700 text-xs font-semibold flex items-center gap-2">
                <i data-lucide="alert-circle" class="w-4 h-4"></i> Perubahan gagal disimpan. Periksa kelengkapan supplier, tanggal, dan item.
            </div>
            <?php endif; ?>

            <!-- Form Card -->
            <form id="purchaseForm" action="process_update.php" method="POST" class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="date" name="tanggal" value="<?= htmlspecialchars($purchase['tanggal']) ?>" required class="ipos-input pl-10">
                        </div>
                    </div>
                </div>
                            ...
                            <div class="col-span-12 sm:col-span-5">
                                <select name="id_barang[]" class="w-full px-3 py-2 border border-slate-200 rounded-lg select-barang text-xs outline-none focus:border-brand-500 bg-white" required>
                                    ...
                                    <?php foreach ($barangs as $b): ?>
                                        <option value="<?= htmlspecialchars($b['id_barang']) ?>" data-price="<?= htmlspecialchars($b['harga_beli']) ?>"><?= htmlspecialchars($b['nama_barang']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="grid grid-cols-12 gap-3 items-center p-3 border border-slate-100 rounded-xl bg-slate-50/50 item-row">
                            <div class="col-span-12 sm:col-span-5">
                                <select name="id_barang[]" class="w-full px-3 py-2 border border-slate-200 rounded-lg select-barang text-xs outline-none focus:border-brand-500 bg-white" required>
                                    <option value="">-- Pilih Produk --</option>
                                    <?php foreach ($barangs as $b): ?>
                                        <option value="<?= htmlspecialchars($b['id_barang']) ?>" data-price="<?= htmlspecialchars($b['harga_beli']) ?>" <?= $b['id_barang'] == $it['id_barang'] ? 'selected' : '' ?>><?= htmlspecialchars($b['nama_barang']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-span-6 sm:col-span-2">
                                <input name="qty[]" type="number" min="1" value="<?= (int)$it['qty'] ?>" required class="w-full px-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-brand-500 font-bold qty-input">
                            </div>
                            <div class="col-span-6 sm:col-span-2 relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[10px] font-bold">Rp</span>
                                <input name="harga_beli[]" type="number" min="0" step="0.01" value="<?= htmlspecialchars($it['harga_beli']) ?>" class="w-full pl-8 pr-3 py-2 border border-slate-200 rounded-lg text-xs outline-none focus:border-brand-500 price-input" placeholder="0">
                            </div>
                            <div class="col-span-8 sm:col-span-2">
                                <div class="subtotal-box px-3 py-2 rounded-lg bg-brand-50 text-brand-700 font-semibold text-right text-xs">Rp 0</div>
                            </div>
